<template>
  ...
  >
    <n-grid :cols="24" :x-gap="24">
      ...
      <n-form-item-gi :span="12" label="询证银行支付系统行号" path="sysBankNumber">
        <n-input
      ...
      <n-form-item-gi :span="12" label="证券公司名称" path="confSenderName">
        <n-input v-model:value="model.confSenderName" placeholder="请输入证券公司名称" />
      </n-form-item-gi>
      <n-form-item-gi :span="12" label="证券公司会员代码" path="securitiesCompanyMemberCode">
        <n-input
          v-model:value="model.securitiesCompanyMemberCode"
          placeholder="请输入证券公司会员代码"
        />
      </n-form-item-gi>
      <n-form-item-gi :span="12" label="证券公司回函地址" path="confReplyerAddress">
        <n-input
          v-model:value="model.confReplyerAddress"
          placeholder="请输入证券公司回函地址"
        />
      </n-form-item-gi>
      <n-form-item-gi :span="12" label="证券公司联系人" path="confReplyerContacts">
        <n-input
      ...
      <n-form-item-gi :span="12" label="授权单位统一社会信用代码" path="acSocialCreditCode">
        <n-input
      ...
      <n-form-item-gi :span="12" label="授权单位联系人" path="acReplyerContacts">
        <n-input
          v-model:value="model.acReplyerContacts"
          placeholder="请输入授权单位联系人"
        />
      </n-form-item-gi>
      <n-form-item-gi :span="12" label="授权单位联系人电话" path="acReplyerPhone">
        <n-input
          v-model:value="model.acReplyerPhone"
          placeholder="请输入授权单位联系人电话"
        />
      </n-form-item-gi>
      <n-form-item-gi :span="12" label="授权单位省/市/县" path="acStateAddres">
        <n-cascader
          placeholder="请选择授权单位省/市/县"
          v-model:value="model.acStateAddres"
          :options="Area"
      ...
      <n-form-item-gi :span="12" label="支取办理询证函回函服务费用的账户" path="paymentAccount">
        <n-input
...
const rules = {
  ...
  confEndDate: [
    { required: false, message: "请选择函证截止时间", trigger: "change" },
  ],
  bankName: [{ required: false, message: "请输入银行名称", trigger: "blur" }],
  sysBankNumber: [{ required: false, message: "请输入询证银行支付系统行号", trigger: "blur" }],
  securitiesCompanyMemberCode: [{ required: false, message: "请输入证券公司会员代码", trigger: "blur" }],
  confReplyerAddress: [{ required: false, message: "请输入证券公司回函地址", trigger: "blur" }],
  confReplyerContacts: [{ required: false, message: "请输入证券公司联系人", trigger: "blur" }],
  confReplyerFax: [{ required: false, message: "请输入回函传真", trigger: "blur" }],
  auditCriteria: [{ required: false, message: "请输入审计准则", trigger: "blur" }],
  acName: [{ required: false, message: "请输入授权公司名称", trigger: "blur" }],
  acSocialCreditCode: [{ required: false, message: "请输入授权单位统一社会信用代码", trigger: "blur" }],
  acLeiCode: [{ required: false, message: "请输入授权单位LEI编码", trigger: "blur" }],
  confSenderName: [{ required: false, message: "请输入证券公司名称", trigger: "blur" }],
  confReplyerPhone: [
    { required: false, message: "请输入回函电话", trigger: "blur" },
  ],
}
/**
